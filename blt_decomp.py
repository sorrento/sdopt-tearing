...
def Dulmage_Mendelsohn(g, eqs):
    ...
    matches = sorted([k for k in mate])
    ...
    for scc in precedence_order:
        equations = eq_sccs._node[scc]['members']
        variables = {n for eq in equations for n in g.adj[eq] if n not in seen}
...
def to_digraph(g, eq_var_matches):
    ...
    for eq, var in eq_var_matches:
        ...
        deps = [n for n in g.adj[eq] if n != var]
        for var_k in deps:
            bipart.add_edge(var_k, eq)
        # eq_k <- var (other equations involving var)
        substitute = [(var, n) for n in g.adj[var] if n != eq]
